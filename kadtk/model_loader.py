...
class MusicFM(ModelLoader):
    ...
    def __init__(self,
        ...
        self.home_dir = Path(__file__).parent / ".model-checkpoints"
        self.model_file = os.path.join(self.home_dir, url_dict['pt'].split('/')[-1])
        self.stat_file = os.path.join(self.home_dir, url_dict['json'].split('/')[-1])
        # Download file if it doesn't exist
        if not os.path.exists(self.model_file) or not os.path.exists(self.stat_file):
            os.makedirs(os.path.dirname(self.model_file), exist_ok=True)
            download_file(url_dict['pt'], self.model_file)
            download_file(url_dict['json'], self.stat_file, progress=False)
    ...
    def _get_embedding(self, audio: np.ndarray) -> torch.Tensor:
        ...
        if audio.shape[0] > self.limit:
            log.warning(f"Audio is too long ({audio.shape[0] / self.sr / 60:.2f} minutes > {self.limit / self.sr / 60:.2f} minutes). Truncating.")
            audio = audio[:self.limit]
        # convert np audio to torch tensor
...
def get_all_models(audio_len: Optional[Union[float, int]] = None) -> list[ModelLoader]:
    ...
    """
    ms = [
        MusicFM('fma', audio_len=audio_len), MusicFM('msd', audio_len=audio_len),
